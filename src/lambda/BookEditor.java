...
public class BookEditor {
	...
	public static void main(String[] args) {
		...
		List<String> result = content.stream().map(s -> s.toUpperCase() + "\n")
				.map(line -> line.substring(0, 1).toUpperCase() + line.substring(1)).collect(Collectors.toList());

		System.out.println(result);

		List<String> resultContent = bookEditor.processText(result);
