...
class Practicum {
	// Задайте форматирование для времени и даты в формате часы:минуты
	// день.месяц.год
	...
	private static void printFlightInformation(String formattedDepartureTime, String departureAirportCode,
		...
		// С помощью класса AirportDatabase получите данные об аэропортах вылета и
		// посадки.
		...
		try {
			...
			System.out.println(
					"Ваш билет на рейс " + departureAirport.getCity() + " - " + arrivalAirport.getCity() + ": ");
			...
			// Найдите время прибытия с учётом зоны прилёта. hs need to check
